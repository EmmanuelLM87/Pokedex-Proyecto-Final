...
const getEvolutions = async (evolutionInfo) => {
    ...
    try {
      ...
      assingInfoToEvolutions(responses, evolutions)
    } catch (err) {
        console.log(err)
    }
...
const assingInfoToEvolutions = (responses, evolutions) => {
    responses.forEach((response, index) => {
        if(responses.status === "fulfilled"){
            evolutions[index].image = response.value.data.sprites.versions["generation-v"][
                "black-white"
            ].front_default;
