...
const PokemonPreview = ({ pokemonURL }) => {
  ...
  return <article className='text-center bg-white rounded-[30px] relative font-semibold 
    capitalize pb-2 pb-4 shadow-lg shadow-slate-400/10 
    border-2 border-transparent hover:border-slate-200 cursor-pointer group grid gap-2'>
    <header className='h-9'>
      <img className='absolute left-1/2 -translate-x-1/2 top-0 -translate-y-1/2 
    ...
    <ul className='flex gap-2 justify-center'>
      {pokemon?.types.map((type) => (<li className={`p-1 rounded-md px-2 text-white text-sm ${colorByType[type.type.name]}`}
        key={type.type.name}>
          {type.type.name}</li>))}
    </ul>
  </article>
